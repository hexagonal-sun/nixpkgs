...
let
  ...
  asyncPatch = fetchurl {
    src = "https://raw.githubusercontent.com/Sporif/dxvk-async/${dxvk.version}/dxvk-async.patch";
    hash = "sha256-3d4HyYBFo7wV+rXq88anVqaktOruxkbUM5FouGrABGM=";
  };

  asyncDxvk = dxvk.override { dxvkPatches = [ asyncPatch ]; };
...
stdenvNoCC.mkDerivation rec {
  ...
  buildPhase = ''
    ...
    declare -A widths=( [1]=256 [2]=48 [3]=32 [4]=16 )
    ...
    for index in "''${!widths[@]}"; do
      ...
      icondir=$res/apps
      mkdir -p $icondir
      mv ffxivboot64.exe_14_103_1041_''${index}_''${res}x32.png $icondir/ffxiv.png
    done
  '';
  ...
  installPhase = ''
    ...
    cp -Rv {??,???}x{??,???} $out/share/icons/hicolor
